...
export const handler: Handler = async (event) => {
  ...
  const { title, author, userId } = JSON.parse(event.body || '{}');
  ...
  if (!title || !author) {
    return { statusCode: 400, body: JSON.stringify({ error: 'Title and author required' }) };
  }
  ...
  try {
    ...
    return { statusCode: 500, body: JSON.stringify({ error: error.message || 'Failed to generate summary' }) };
  }
};
